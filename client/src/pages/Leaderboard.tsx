          ...
          <table className="w-full">
            ...
            <tbody>
              {standings.map((row, idx) => (
                <tr key={row.name} className={`table-row ${idx === 0 ? 'border-l-4 border-l-cyber-yellow' : ''}`}>
                  <td className="table-cell">
                    {idx === 0 && (
                      <span className="badge-rank">{idx + 1}</span>
                    )}
                    {idx !== 0 && <span className="text-cyber-muted">{idx + 1}</span>}
                  </td>
                  <td className="table-cell font-bold">{row.name}</td>
                  <td className="table-cell text-cyber-muted">{row.school}</td>
                  <td className="table-cell text-right text-cyber-green">{row.uptimePercent}</td>
                  <td className="table-cell text-right text-cyber-green">{row.injectsTotal.toFixed(1)}</td>
                  <td className="table-cell text-right text-cyber-accent font-bold">{row.total.toFixed(1)}</td>
                </tr>
              ))}
            </tbody>
          </table>
